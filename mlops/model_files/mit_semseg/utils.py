...
def find_recursive(root_dir, ext=['.jpg', '.jpeg', '.gif', '.png']):
    files = []
...
#                     (labelmap.shape[0], labelmap.shape[1], 1))
        ...
        #fix error: Cannot cast ufunc 'add' output from dtype('int64') to dtype('uint8') with casting rule 'same_kind'
        labelmap_rgb = np.add(labelmap_rgb, (labelmap == label)[:, :, np.newaxis] * \
            np.tile(colors[label],
                    (labelmap.shape[0], labelmap.shape[1], 1)), out=labelmap_rgb, casting="unsafe")
