...
class TrainDataset(BaseDataset):
    ...
    def __getitem__(self, index):
        ...
        for i in range(self.batch_per_gpu):
            ...
            # if not None, preprocess with specified filter
            if self.preprocess != None:
                ...
                #apply filter
                if self.preprocess == "frangi":
                    img = filters.frangi(img)
                elif self.preprocess == "hessian":
                    img = filters.hessian(img)
                elif self.preprocess == "meijering":
                    img = filters.meijering(img)
                elif self.preprocess == "sato":
...
class TestDataset(BaseDataset):
    def __init__(self, odgt, opt, **kwargs):
        ...
        self.flip = opt.flip
        self.scaling_perc = opt.scaling_perc
        self.border_padding = opt.border_padding
    ...
    def __getitem__(self, index):
        ...
        if self.rotate == "rotate_counterclockwise":
            img = img.rotate(90, expand = True)
        
        if self.rotate == "rotate_clockwise":
        ...
        if self.preprocess == "resize":
            scale = self.scaling_perc
            new_w, new_h = int(img.size[0]*scale), int(img.size[1]*scale)
            img = img.resize((new_w,new_h), Image.ANTIALIAS)
            
        if self.preprocess == "resize_and_padding":
            scale = self.scaling_perc
